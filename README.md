# Hello there, I'm Neil! 👋 

## "Who am I?"
I am a 20-year-old college student from Chicago with over seven years of programming experience. As a rising junior, I major in both Computer Science and Statistics & Machine Learning at [Carnegie Mellon University](https://cs.cmu.edu). I am deeply interested in machine learning systems, data engineering, and applied ML (especially in fields such as computer vision and NLP). I strive to learn something new each month, and I try to do so by participating (& winning!) in hackathons, working on side projects, or simply talking about newly released tech. I love to build, starting from the idea to the finishing touches.

## "Cool, what do you know?"
In practice, my tech stack is quite basic: React, Python + FastAPI, and some flavor of SQL for database management. In theory, I know C/C++, OCaml, Java, R, and SML; however, I seldom use these languages in applications (maybe someday I can apply them). Lastly, I am a big Unix user, where I run all my applications on an M4 Max MacBook Pro. My specific skills are not listed here.

## "What are you up to, besides building?"
Currently, I'll be working as a Software Engineer intern at Capital One this summer in their Chicago office. I've also made it a priority to strengthen my piano skills, as I've lost most of it by not practicing.

### Some Important Links
Portfolio & Blog → [https://www.neilpuroh.it/](https://neilpuroh.it/) <br>
LinkedIn → [/in/neilpurohit06](https://linkedin.com/in/neilpurohit06) <br>
Contact Me → [me@neilpuroh.it](mailto:me@neilpuroh.it) <br>
